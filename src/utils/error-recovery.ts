...
export class ErrorRecoveryManager {
  ...
  async executeWithRecovery<T>(
    ...
    try {
      ...
      this.logger.logError(enhancedError, `${context.toolName}_${context.operationName}`, {
        tool: context.toolName,
        operation: context.operationName,
  ...
  private async executeWithRetry<T>(
    ...
    while (attempt < this.config.retry.maxAttempts) {
      try {
        ...
        if (!this.isRetryableError(lastError) || attempt >= this.config.retry.maxAttempts) {
          throw lastError;
        }

        const delay = this.calculateDelay(attempt);

        this.logger.warn(`Operation ${context.operationName} failed, retrying in ${delay}ms`, {
          attempt,
  ...
  private enhanceError(error: Error, context: { operationName: string; toolName: string }): Error {
    ...
    const errorMappings = [
      {
        ...
        userMessage: 'The service appears to be unavailable. Please try again later.',
      },
      {
        pattern: /401|Unauthorized/,
        type: 'AUTHENTICATION_ERROR',
        message: 'Authentication failed. Please check your credentials.',
        userMessage: 'Your authentication credentials may have expired. Please check your API tokens.',
      },
      {
      ...
      },
      {
        ...
        userMessage: 'The request parameters are invalid. Please check your input.',
      },
      {
        pattern: /500|Internal Server Error/,
        type: 'SERVER_ERROR',
        message: 'Internal server error on the remote service.',
        userMessage: 'The remote service is experiencing issues. Please try again later.',
      },
      {
    ...
    for (const mapping of errorMappings) {
      if (mapping.pattern.test(error.message)) {
        return new BaseError(
          ...
          false,
          mapping.userMessage,
          { originalError: error.message, operation: context.operationName, tool: context.toolName }
        );
      }
    }
  ...
  private async handleGracefulDegradation<T>(
    error: Error,
    context: { operationName: string; toolName: string; fallback?: () => Promise<T> }
  ): Promise<T> {
    ...
    if (context.operationName.includes('search') || context.operationName.includes('get')) {
  ...
  private isCircuitBreakerOpen(key: string): boolean {
    ...
    if (now - state.lastResetTime > this.config.circuitBreaker.monitoringPeriod) {
    ...
    if (!state.isOpen && state.failureCount >= this.config.circuitBreaker.failureThreshold) {
      state.isOpen = true;
      state.openedAt = now;
    ...
    if (state.isOpen && now - state.openedAt! > this.config.circuitBreaker.timeout) {
      state.isOpen = false;
      this.logger.info(`Circuit breaker closed for ${key}`, { timeoutPeriod: this.config.circuitBreaker.timeout });
    }
  ...
  private getCircuitBreakerState(key: string): Partial<CircuitBreakerState> | undefined {
    return this.circuitBreakers.get(key);
  }

  // Utility methods
  private isRetryableError(error: Error): boolean {
    return this.config.retry.retryableErrors.some(errorType => error instanceof errorType) ||
...
export class ErrorAnalytics {
  ...
  recordError(error: Error, context: { tool: string; operation: string }): void {
  ...
  getErrorSummary(): {
    ...
    const recommendations = this.generateRecommendations(topErrorPatterns, errorsByOperation);
  ...
  private extractErrorPattern(error: Error): string {
    ...
    if (message.includes('permission') || message.includes('forbidden')) return 'authorization_error';
  ...
  private generateRecommendations(
    ...
    for (const { pattern, count } of topPatterns) {
      if (count > 10) { // Threshold for generating recommendations
        switch (pattern.split(':')[1]) {
          case 'network_error':
            recommendations.push('Consider implementing connection pooling and health checks for network stability');
            break;
          case 'timeout_error':
            recommendations.push('Review timeout configurations and consider implementing circuit breakers');
            break;
          case 'rate_limit_error':
            recommendations.push('Implement intelligent rate limiting with backoff strategies');
            break;
          case 'authentication_error':
            recommendations.push('Review API credentials and implement token refresh mechanisms');
            break;
          case 'validation_error':
            recommendations.push('Enhance input validation and provide better error messages to users');
            break;
        }
      }
    }
